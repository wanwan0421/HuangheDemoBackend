...
export class LlmAgentController {
  ...
  @Post('explainRecommendation')
  async explainRecommendation(
    @Body('md5') md5: string,
    @Body('task') task?: string,
  ) {
    if (!md5) {
      throw new HttpException('Model md5 is required', HttpStatus.BAD_REQUEST);
    }
    const explanation = await this.llmAgentService.explainRecommendedModel(md5, task);
