...
import {
  indexRecommendationTool,
  modelRecommendationTool,
} from './schemas/llmTools.schema';
...
export class LlmAgentService {
   ...
   * @returns 推荐的5个指标信息
   */
  private async reconmmendIndex(
    prompt: string,
    userQueryVector: number[],
  ): Promise<LlmRecommendationResponse | null> {
    // 进行向量搜索，获取相近的10个指标信息
    const relevantIndex =
      await this.indexService.findRelevantIndex(userQueryVector);
  ...
  public async reconmmendModel(prompt: string) {
    ...
    // 指标推荐
    const indexRecommendation = await this.reconmmendIndex(
      prompt,
      userQueryVector,
    );
    ...
    console.log(
      '指标信息:',
      JSON.stringify(indexRecommendation.recommendations),
    );
    ...
    // 进行向量搜索，获取相近的5个模型的名称
    const relevantModel = await this.indexService.findRelevantModel(
      userQueryVector,
      modelIds,
    );

    // 获取5个模型的详细信息
    const fetchModelDetails = relevantModel.map((index) =>
      this.resourceService.getModelDetails(index.modelMd5),
    );
    ...
    const simpleModelList = modelDetailsResults
      .filter(
        (item): item is ModelResource => item != null && item !== undefined,
      )
      .map((model) => {
        return {
          name: model.name,
    ...
    const contents = [
      {
        ...
        parts: [
          {
            text: `You are a professional expert in recommendation of geographical model. 
    ...
    try {
      ...
      if (response.functionCalls && response.functionCalls.length > 0) {
        ...
        if (functionCall.name === 'recommend_model') {
          ...
          // 再从数据库获取最终模型的完整详情
          const finalModel =
            await this.resourceService.getModelDetails(recommendModelMd5);

          if (finalModel && finalModel.data?.input) {
            const workflowSteps = finalModel.data.input.map(
              (state, stateIndex) => {
                return {
  ...
  async explainRecommendedModel(md5: string, task?: string): Promise<string> {
    const model = await this.resourceService.getModelDetails(md5);
    if (!model) {
      return '暂时无法获取该模型的详细信息，因此还不能准确说明推荐原因。';
    }

    const modelName = String(model.name || '该模型').trim();
    const description = String(model.description || '').trim();
    const evidence = {
      name: modelName,
      description,
      input: model.data?.input || model.inputParams || [],
      output: model.data?.output || model.outputParams || [],
      mdl: model.mdlJson?.mdl || model.mdl || null,
    };
    const serializedEvidence = JSON.stringify(evidence, null, 2).slice(0, 18000);
    const hasDetails =
      description.length >= 12 ||
      JSON.stringify(evidence.input).length > 20 ||
      JSON.stringify(evidence.output).length > 20 ||
      JSON.stringify(evidence.mdl).length > 30;

    if (!hasDetails) {
      return `目前关于 ${modelName} 的功能、输入输出和适用场景说明较为有限，暂时无法准确说明它适合当前任务的具体原因。`;
    }
    ...
    const response = await this.genAIService.generateContent(
      [{
        role: 'user',
        parts: [{
          text: `你是一名地理模型说明助手。模型已经由另一个系统选定，你只负责解释结果，绝对不能重新选择模型。\n\n用户任务：${task || '未提供'}\n\n模型资料：\n${serializedEvidence}\n\n请用中文写 3 至 5 句话的推荐说明：\n1. 说明该模型的用途或性质，以及为什么与用户任务有关；\n2. 结合资料中明确存在的输入、输出或 MDL 过程说明其适用性；\n3. 可以说明优势，但每个优势必须能从给出的名称、描述、输入、输出或 MDL 中直接得到支持；\n4. 严禁补充资料中没有的算法、精度、尺度、性能或应用能力；\n5. 不得提及检索、排名、候选、得分、数据库、推理过程或“资料显示”；\n6. 某项信息缺失时不要猜测，也不要讨论缺失字段；只输出面向用户的说明正文，不使用标题或列表。`,
        }],
      }],
      null,
    );

    const explanation = String(response.text || '').replace(/^#+\s*/gm, '').trim();
    return explanation || `目前关于 ${modelName} 的说明较为有限，暂时无法给出更详细的推荐原因。`;
  }
}
